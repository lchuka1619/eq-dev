...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const activePath = pathname.startsWith("/practice/") ? "/today" : pathname;
  return (
    <div className="app-shell">
      <header className="app-header">
        ...
        <nav className="desktop-app-nav" aria-label="Дасгалын үндсэн цэс">
          {items.map((item) => <Link key={item.href} href={item.href} aria-current={activePath === item.href ? "page" : undefined}>{item.label}</Link>)}
        </nav>
        <AccountMenu />
      </header>
