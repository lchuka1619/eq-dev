...
**Огноо:** 2026-07-24  
**Сүүлийн шинэчлэлт:** 2026-07-24 — public landing/app shell route contract, auth destination, guest flow
...
### 1.1 Public site ба Personal Practice нь нэг page биш

Одоогийн `b73b056` кодод `/` route нь public hero, Today router, Arena, voice coach, practice, role-play, progress-ийг нэг урт page дээр render хийдэг. Login хийсний дараа зөвхөн account control өөрчлөгдөж, хэрэглэгч app руу орсон гэдгээ мэдрэхгүй.

Зорилтот бүтэц:

| Route | Үүрэг | Эхний харагдах зүйл |
|---|---|---|
| `/` | Public landing | Үнэ цэнийн амлалт + `Дасгал эхлэх` |
| `/today` | Personal Practice home | Нэг recommended Today card |
| `/journey` | Замнал | Current 7-day plan |
| `/progress` | Ахиц | Streak, session, mastery |
| `/profile` | Профайл | Account, sync, privacy |

Landing нь бүтээгдэхүүнийг тайлбарлана. App shell нь хэрэглэгчийг дасгал хийлгэнэ. Auth бол энэ хоёрын харагдах ялгааг орлохгүй.

### 1.2 Entry ба auth destination дүрэм

```mermaid
flowchart TD
    A["Visitor opens /"] --> B{"Session байна уу?"}
    B -- "Үгүй" --> C["Public landing"]
    C -->|Дасгал эхлэх| D["/today · guest"]
    C -->|Нэвтрэх| E["Auth"]
    E -->|Success| F["/today"]
    B -- "Тийм" --> F
    D -->|Progress хадгалах| E
```

- Guest хэрэглэгч `/today`-д auth шаардахгүй.
- `/today`, `/journey`, `/progress`, `/profile` дотроос login хийвэл тухайн safe internal route руу буцна.
- `/`-ээс login хийвэл `/today` руу орно.
- Authenticated хэрэглэгч `/` нээвэл `/today` руу шилжинэ.
- Sign out хийвэл `/today` guest mode хэвээр ажиллана.
- Invalid/external `next` нь `/today` руу fallback хийнэ; open redirect хориглоно.
- Session restore хийх зуур landing → app content flash гаргахгүй.
...
Navigation нь зөвхөн document anchor биш, route-aware app navigation байна. Active item дээр `aria-current="page"` хэрэглэнэ. Mobile дээр safe-area inset-ийг хүндэтгэсэн bottom navigation; desktop дээр ижил дөрвөн destination-ийг compact top/side navigation-аар харуулж болно.
...
|---|---|---|---|
| P01 | Splash / session restore | Neutral loading; session/local/cloud state сэргээж зөв route сонгох; content flash үгүй | Тийм |
| P02 | Public landing (`/`) | “Зайлсхийж буй яриагаа аюулгүй давт” амлалт; **Дасгал эхлэх**, **Нэвтрэх** | Тийм |
...
| P12A | Auth destination | Safe internal `next`; default `/today`; auth error үед guest practice хаахгүй | Тийм |
...
**Route acceptance:** login амжилтын нотолгоо нь зөвхөн avatar биш. Хэрэглэгч practice-first app shell дээр орж, active navigation болон Today content-оор app төлөвөө шууд ойлгоно.

### B. Today / Home

| ID | Дэлгэц | Гол агуулга / үйлдэл | MVP |
|---|---|---|---|
| P13 | Today (`/today`) | App shell-ийн эхний viewport-д нэг том “Өнөөдрийн дасгал” card; хугацаа, шалтгаан, streak, 3/7 progress | Байгаа/route салгана |
...
`/` нь canonical public landing. `/today`, `/journey`, `/progress`, `/profile` нь Personal Practice app shell-ийн canonical top-level destination байна. Хуучин `/welcome` эсвэл `/journeys` route байвал canonical route руу redirect хийж болно; navigation дотроос давхар route бүү үүсгэ.
...
Бүх дэлгэцийг нэг дор хийхгүй. Дараагийн бодит pilot-д дараах үндсэн дэлгэц/төлөв хангалттай:
...
Эдгээрийн гадна нэг reusable **Personal Practice app shell** байна. Энэ нь тусдаа контентын дэлгэц биш; `/today`, `/journey`, `/progress`, `/profile` route-ын shared header/navigation/layout юм.
...
- Public `/` landing-ийг Personal Practice UI-аас салгах
- Guest болон authenticated хэрэглэгчийн canonical entry-г `/today` болгох
- Auth callback, safe `next`, session restore, sign-out destination-ийг баталгаажуулах
- Navigation-ийг route-aware 4 tab болгох
